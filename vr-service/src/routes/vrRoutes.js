...
const toAbsoluteModelUrl = (modelPath, backendOrigin) => {
  ...
  // Prefer same-origin paths for frontend/ngrok single-tunnel usage.
  const normalizedPath = modelPath.startsWith('/') ? modelPath : `/${modelPath}`;

  // Avoid leaking Docker-internal hostnames to browsers.
  ...
  try {
    ...
    if (!modelPath) {
      return res.status(422).json({
        success: false,
        message: 'No se pudo resolver un modelPath para la sesion VR'
      });
    }

    return res.status(200).json({
      success: true,
      message: 'Sesion VR preparada',
      data: {
    ...
    return res.status(500).json({
      success: false,
      message: 'Error preparando la sesion VR',
